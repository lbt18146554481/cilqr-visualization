...
// On macOS and many platforms, int64_t is typically an alias for long long,
// so defining both causes template specialization redefinition errors.
// Only define these if they're actually different types.
// For now, comment them out to avoid redefinition errors - int64_t/uint64_t should cover the use cases.
...
inline bool plot(const Eigen::VectorXd& x, const Eigen::VectorXd& y,
    ...
    for (std::map<std::string, std::string>::const_iterator it = keywords.begin();
        ...
        if (it->first == "linewidth" || it->first == "alpha" || it->first == "zorder") {
            // Use PyLong_FromLong for Python 3 compatibility, and convert to int
            PyDict_SetItemString(kwargs, it->first.c_str(), PyLong_FromLong(std::stol(it->second)));
        } else {
            PyDict_SetItemString(kwargs, it->first.c_str(),
                                 PyString_FromString(it->second.c_str()));
        }
